        ...
        Data::Struct(s) => {
            ...
            for (i, field) in s.fields.into_iter().enumerate() {
                ...
                if ctx_ident.is_none() {
                    ctx_ident = Some(quote_spanned!{span=> <#ty as genetic_rs_common::prelude::RandomlyMutable>::Context });
                }
...
fn create_context_helper(ast: &DeriveInput, trait_name: syn::Ident) -> (Option<proc_macro2::TokenStream>, Option<proc_macro2::TokenStream>) {
    let name = &ast.ident;
    ...
    if let Some(ident) = args.create_context {
        match &ast.data {
            Data::Struct(s) => {
                ...
                if tuple_struct {
                    return (Some(quote!{ #doc #vis struct #ident (#inner);}), Some(ident.to_token_stream()))
                }
    ...
    match ast.data {
        Data::Struct(s) => {
            ...
            for (i, field) in s.fields.into_iter().enumerate() {
                ...
                if context.is_none() {
                    context = Some(quote! { <#ty as genetic_rs_common::prelude::Crossover>::Context });
                }
